...
이 디렉터리는 팀 **옷개스트라**의 자동 의류 정리 로봇 시스템 **「접신」**에서 NVIDIA Jetson Orin Nano를 기반으로 실행되는 **AI 기반 컴퓨터 비전, 의류 상태 분석, 조작 계획 및 듀얼 RoArm M2-S 제어 소프트웨어**를 관리합니다.
...
현재 커스텀 모델은 7개 동작 클래스를 학습했으며, 제출용 실행 코드에서는 신뢰도와 margin 조건을 만족한 `CENTER`, `SPREAD`, `ROTATE`, `FINISH` 판단을 우선 직접 사용합니다. 그 외 경우에는 기존 Auto-X 판단으로 넘어갑니다.

즉, 현재 상의 코드는 **커스텀 모델을 우선 사용하여 동작을 판단하고, 모델 판단을 직접 채택하기 어려운 경우 기존 Auto-X 판단 로직을 보조 경로로 사용**합니다.

실제 파지점과 이동 경로는 기존 검증된 조작 계획 및 안전 검사 로직을 통해 최종 결정됩니다.

조작이 끝나면 로봇팔이 Standby 위치로 복귀한 뒤 새로운 영상을 다시 관찰하고 필요한 동작을 반복합니다. `FINISH`가 선택되면 해당 의류의 자동 조작 루프를 종료합니다.
---
...
상의는 현재 **학습 모델 우선 판단 + Auto-X 보조 판단**을 이용한 자동 반복 조작 구조를 적용하고 있으며, 하의는 VLA 학습을 위한 데이터 수집 및 자동화 개발을 진행하고 있습니다.
---
...
학습 원본인 `best.pt`와 변환 중간 파일인 `top_board_state_v2_fp32.onnx`는 모델 백업 및 재변환을 위해 별도로 보관합니다. Jetson의 실제 동작 결정 추론에서는 TensorRT 엔진과 `state_normalization.npz`를 사용합니다.
...
- 커스텀 모델 판단을 우선 사용하고 필요한 경우 기존 Auto-X 판단으로 보완
...
현재 **상의는 학습 기반 동작 결정 모델과 Auto-X 보조 판단을 이용하여 의류 상태를 반복 관찰하고 필요한 조작을 자동 수행한 뒤 FINISH까지 판단하는 구조를 구현**했습니다.
