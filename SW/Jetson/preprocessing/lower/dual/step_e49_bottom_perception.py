...
# -*- coding: utf-8 -*-
"""
Bottom-garment perception and geometry module.

This module provides:
  - segmentation and garment-mask selection
  - empty-board baseline validation
  - strict ArUco-board ROI sanitation
...
Segmentation masks are constrained to the configured ArUco-board region, marker
patches are excluded, and connected-component/Pose-consistency sanitation is applied.
In strict ROI mode, an invalid board ROI never falls back to an unsanitized mask.
"""
...
def pose_tta_summary(kxy: np.ndarray, kcf: Optional[np.ndarray], min_conf: float,
    ...
    # Geometry validation may rank or reject a candidate, but it does not
    # rewrite waist/crotch/hem semantic landmarks.
