...
E60 bottom-garment confidence-gated late fusion perception.

E60 keeps Pose semantic landmarks independent, retries segmentation inside a
Pose-guided crop when full-frame segmentation returns no usable mask, and
separates robot-action masks from display-only diagnostic masks/Heatmaps.

E60 additionally applies a conservative structural-semantic refinement pass to
K0~K7.  It preserves the raw network output, enforces garment-local left/right
ordering, validates waist/crotch/hem topology, and uses a valid segmentation
contour only for local boundary snapping.  It never changes waist/hem polarity.

E60 preserves E49 empty-board / strict ArUco-board ROI sanitation and adds a
waist-semantic gate for supervised WAIST_L/WAIST_R grasping.  The gate does not
change the eight visual keypoints.  It separately reports whether points 0/1/2
are safe enough to drive a physical internal-grasp test lift.
...
class BottomPoseBoard:
    ...
    # E59: preserve model output and expose per-keypoint semantic provenance.
...
class BottomObservation:
    ...
    # E52: separate physical-waist-grasp eligibility from general observation validity.
    waist_semantic: Optional[Dict[str, Any]] = None
    waist_lift_usable: bool = False
    # E60 late-fusion diagnostics. `mask`/`wrinkle` remain action-only so older
    # policy/planner code can never consume a display fallback accidentally.
    ...
    # E60 multimodal second-stage completion state. Spread readiness uses
    # action-safe Segmentation + wrinkle Heatmap + Pose shape/leg topology.
...
# =============================================================================
# E60: D19/D27-inspired finish heatmap
#   - remove pose-known waistband/crotch natural structure zones
